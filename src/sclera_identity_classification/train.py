...
parser.add_argument("-sm", "--model_save_path", type=str, default='models/model', help="path to save model file")
...
parser.add_argument("-bs", "--batch_size", type=int, default=96, help="batch size")
...
##########################  Revised structure with config files #############################################################
...
def train(config, net, val_loader):
...
def main(config):
    # Prepare the data:
    _, val_loader, test_loader = prepare_data(config)

    # Initialize the model/network:
    net = init_net(config)

    # Train model:
    net, losses = train(config, net=net, val_loader=val_loader)
...
if __name__ == "__main__":
    ...
    #net, losses = train(net, args.epochs)


#torch.save(net, model_save_path + ".pth")
